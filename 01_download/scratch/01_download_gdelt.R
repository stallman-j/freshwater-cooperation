...
# bring in the packages, folders, paths ----
  ...
  if (!require("crancache")) devtools::install_github("briatte/crancache")
...
# Global Knowledge Graph ----
  library(lubridate)
  if (!require("crancache")) install.packages("crancache")
  ...
  end_date        <- "20180326133000"
  
  
  #end_date        <- "20181231234500"
  
  #  user  system elapsed until 2018-03-25 13:30:00
  # 16.14  120.04 4847.97 
  ...
  for (i in seq_along(dates)){
    destfile_path <- file.path(data_external_raw,"GDELT","gkg",type,filenames[i])
  ...
  unzip(file.path(data_external_raw,"GDELT","gkg",type,paste0(i,file_types)),
        exdir = file.path(data_external_raw,"GDELT","gkg",type),
        junkpaths = TRUE,
        overwrite = TRUE)
  file.remove(file.path(data_external_raw,"GDELT","gkg",type,paste0(i,file_types)))
  }
  
  # started February around 12:45 pm
  # finished February around 1:15pm
  
  
  
 
    # system time for 1 day downloading the "gkg" type
  # user  system elapsed 
  # 0.12    1.06   40.14
  
  saveRDS(missing_urls,
          file = file.path(data_external_raw,"GDELT","gkg","missing-indices","missing_urls_gkg_20180102000000_to_20180325131500.rds"))
  
  saveRDS(missing_urls,
          file = file.path(data_external_raw,"GDELT","gkg","missing-indices",paste0("missing_urls_",type,"_",start_date,"_to_",actual_end,".rds")))
  
  # 21464 records downloading the "export"
  # user  system elapsed 
  # 9.94   39.88 5019.34
