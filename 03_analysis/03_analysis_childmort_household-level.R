...
era5_gps_childmort <- readRDS(file.path(data_external_clean,"merged",paste0("DHS_HH_infant_mortality_GPS_ERA5_rivers_1_to_3_towns.rds"))) %>% as.data.frame() %>%
                      mutate(dist_to_riv_km = as.numeric(distance_to_river)/1000) %>%
                      ...
                      group_by(MAIN_RIV) %>%
                      mutate(n_towns = length(unique(DHSID))) %>%
                      ungroup() %>%
                      group_by(year,DHSID) %>%
                      mutate(n_children_town_year = n()/2) %>% ungroup() %>%
                      mutate(river_length_km = DIST_UP_KM + DIST_DN_KM,
                             infant_death_1000_exposure = infant_death_1000*share_year_t) %>%
                      mutate(rural = if_else(URBAN_RURA == "R",
                                             1,
                                             0)) %>%
                      filter(dist_to_riv_km <= 10) %>%
                      filter(DHSCC != "MD") %>% # take out madagascar for now
                      filter(river_length_km <= 150)


era5_gps_childmort_1_town <- era5_gps_childmort %>% filter(n_towns == 1)
era5_gps_childmort_2_town <- era5_gps_childmort %>% filter(n_towns == 2)
era5_gps_childmort_3_town <- era5_gps_childmort %>% filter(n_towns == 3)

length(unique(era5_gps_childmort$DHSID))
length(unique(era5_gps_childmort$DHSID))

length(unique(era5_gps_childmort_1_town$DHSID))
...
for (i in 1:2){
  if (i==1) {
    current_data     <- era5_gps_childmort
    filestub <- "01_to_03_town_rivers"
    name_stub     <- " 1- to 3-Town Rivers"
  } else {
    current_data = era5_gps_childmort_1_town
    filestub <- "01_town_rivers"
    name_stub     <- " 1-Town Rivers"
  }
...
ggplot(mapping = aes(x = era5_gps_childmort_1_town$DHSCC)) +
...
## set up regression formulas
  ...
  reg_1_vars  <- c("precip_annual_zscore_mm_month")
  reg_2_vars  <- c("precip_annual_zscore_mm_month","I(precip_annual_zscore_mm_month^2)")
  reg_3_vars  <- c("precip_annual_zscore_mm_month","precip_005y_ra")
  reg_4_vars  <- c("precip_annual_zscore_mm_month","precip_005y_ra","rural","precip_005y_ra:rural")
  reg_5_vars  <- c("precip_annual_zscore_mm_month","precip_005y_ra","DIST_UP_KM")
  reg_6_vars  <- c("precip_annual_zscore_mm_month","precip_005y_ra","DIST_UP_KM","precip_005y_ra:DIST_UP_KM")
  reg_7_vars  <- c("precip_annual_zscore_mm_month","precip_005y_ra","precip_005y_ra:DIST_UP_KM")
  

  
reg_1_form <- reg_equation(outcome_var = "infant_death_1000",
                           regressor_vars = reg_6_vars,
                           fe_vars = NULL)

reg_2_form <- reg_equation(outcome_var = "infant_death_1000",
                           regressor_vars = reg_7_vars,
                           fe_vars = "DHSID")

reg_3_form <- reg_equation(outcome_var = "infant_death_1000",
                           regressor_vars = reg_7_vars,
                           fe_vars = "year")

reg_4_form <- reg_equation(outcome_var = "infant_death_1000",
                           regressor_vars = reg_7_vars,
