...
# search parameters ----
  ...
  query_words<- '"water security"'
  #(hydropower dam OR hydropower OR dam OR hydro)"' # OR Cuando OR Lunge-Bungo OR Lungwebungu OR Kabompo OR Dongwe OR Lunga OR Luwishi OR Kafue OR Zambezi OR Machili OR Kariba OR Shangani OR Gwai OR Mazowe OR Sanvati OR Lake Cabora Bassa OR Shre OR Luangwa )"' # OR freshwater OR fresh water OR treatment plant OR wastewater OR water quantity OR flood OR drought)'"
  
  source_countries <- c("ZI","MI","ZA","BC")

  query_locations <-"zimbabwe"  #'"(locationcc:ZI OR locationcc:MI OR locationcc:ZA OR locationcc:BC)"' #'(Zimbabwe OR Malawi OR Angola OR Mozambique OR Botswana OR Tanzania)'"
...
# loop through the queries ----
    ...
    URL_p2 <- "-sourcelang:"
    URL_p3 <- "&mode=pointdata" # "&mode=artlist&maxrecords=250&sort=datedesc&startdatetime="
    URL_p4 <- "&format=CSV"
    URL_p5 <- "&timespan = 1w&sortby=Date"
...
#Deduplicate, check, and export data ----
  ...
  filename <- paste0(Sys.Date(),"_geo_query.csv")
