#' Plot River and Points
#' @description from points_data which has lat, long, an id variable, and country assigned, as well as a main_river which defines a river network, plot the points along the river and within its country
...
plot_river_and_points <- function(main_river,
                             lat_varname = "lat",
                             long_varname = "lon",
                             id_varname   = "ID",
  ...
  checked_river_path         <- file.path(data_temp_path,"merged","DHS_GLOW_HydroSHEDS","checked-main-rivers","plotted-rivers")
  ...
  if (!file.exists(file.path(river_points_path,paste0("DHS_GLOW_MAIN_RIV_",main_river,"_river_points.rds")))){
      
      stop(paste0("River points data in ",river_points_path," doesn't exist. Create it with get_river_points(",main_river,"). Then come back to plot"))
      
    }
  
  points_data <- readRDS(file =file.path(river_points_path,paste0("DHS_GLOW_MAIN_RIV_",main_river,"_river_points.rds")))
  ...
  if (nrow(points_data)==0){
    ...
    stop(paste0("No datasets intersect with MAIN_RIV ",main_river,"."))
  }
