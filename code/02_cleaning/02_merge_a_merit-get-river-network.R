...
# bring in the packages, folders, paths ----
  ...
  n_cores          <- 10 #detectCores(logical = TRUE) - 2
...
# bring in hydrorivers ----
  ...
  path <- file.path(data_external_raw,"MERIT_Hydro","hnd_s60e150.tar")
  
  system.time(
    merit_rivers <- st_read(dsn = path) %>% 
  ...
  temp <- merit_rivers %>% filter(MAIN_RIV==10071782)
  ...
  #merit_rivers_units <- readRDS(file = file.path(out_path,paste0("SEN_hydrorivers.rds")))
  
  #main_rivers_senegal <- merit_rivers_units$MAIN_RIV %>% unique() %>% .[-c(1)] # got this from restricting units to Senegal later on
  
  # get all the main rivers and convert to a vector to loop through
  main_rivers_all <- merit_rivers %>% 
                    st_drop_geometry() %>%
                     dplyr::select(MAIN_RIV) %>%
                    unique() %>% as.vector() %>% .[[1]]
  
  
  # main_rivers_sizes <- merit_rivers %>%
  #                    st_drop_geometry() %>%
  #                    group_by(MAIN_RIV) %>%
  ...
  # 
  # main_rivers_medium_big <- merit_rivers %>%
  #                           st_drop_geometry() %>%
  #                           group_by(MAIN_RIV) %>%
  #                           summarise(n_segments = n()) %>%
  #                           ungroup() %>%
...
# parallelize making the river network ----
  ...
  get_river_network <- function(current_main_river,
                                ...
                                #merit_rivers = merit_rivers # DO NOT Uncomment this - including it caused a recursive error
                                ){
  ...
  clusterExport(cl, c("merit_rivers")) # this is a big export b/c the merit_rivers is huge
