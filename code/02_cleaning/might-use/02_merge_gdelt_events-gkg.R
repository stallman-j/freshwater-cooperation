...
# merge the events and gkg water for particular dates ----
  ...
  merge_events_gkg_water <- function(theme,
                                     gkg_input_filename = "_gkg_water_related.rds",
                                     gkg_input_path         = file.path("C:","data_C_drive","02_temp","GDELT","gkg","gkg","full","filtered","africa-country-theme"),
                                     events_input_filename = "_events_full.rds",
                                     events_input_path      = file.path("E:","data","01_raw","GDELT","events","full"),
  ...
  # fix paths----
    ...
    input_path <- file.path(input_path,"yearly",year)
    
    
  # bring in the events
  events_tmp <- readRDS(file = paste0(events_input_path, "/",paste0(date,events_input_filename))) %>%
    filter(!is.na(urlSource))

  # a day is like 5-80 MB
  
  gkg_water_theme   <- readRDS(file = paste0(gkg_input_path,"/",paste0(date,gkg_input_filename))) %>%
                  filter(!is.na(documentSource))
  
  # Merge by URL: gkg is "documentSource" and for events is "urlSource"
  # "merge" variable documents what kind of match we get
