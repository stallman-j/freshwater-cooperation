...
# bring in hydrorivers ----
  ...
  temp <- hydro_rivers %>% filter(MAIN_RIV==main_river)
...
# parallelize making the river network ----
  ...
  saveRDS(current_main_river,file = file.path(out_path,"long_rivers",paste0("long_river_MAIN_RIV_",current_main_river,".rds")))
    } else {
  ...
  # Get river networks for all the HydroRIVERS: 321422.05 sec elapsed for 8 rivers of lenght:
  # 1
  # 11110394
  # 4045
  # 2
