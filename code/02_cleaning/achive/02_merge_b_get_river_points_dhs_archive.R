...
# choose countries or rivers to include ----
  ...
  dyads <- distance_data %>%
    filter(n_towns == 2) %>% 
    filter(upstream!=downstream) %>% # take out the own-node observation 
    mutate(same_river_segment = if_else(distance_m == 0,
                                        true = 1,
                                        false = 0))
  
  dyad <- distance_data %>% filter(n_towns == 2)
...
# parallelize ----
  ...
  parLapply(cl, main_rivers_singletons, get_dhs_river_distances)
