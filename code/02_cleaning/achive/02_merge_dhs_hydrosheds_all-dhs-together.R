...
# bring in hydrorivers ----
  ...
  main_rivers_df <- main_rivers_df %>%
                    mutate(checked = 0) %>%
                    mutate(checked = ifelse(row_number()< 210, 1, 0))
...
# get the DHS locations placed on river networks ----
  ...
  for (j in 1:length(main_rivers_all)){
    ...
    tic(paste0("Ran through entire sequence with main_river ",main_river))
    ...
    for (year in years){
      ...
      print(paste0("Working on DHS for year ",year, " of years ",paste(years, collapse = " ")," in ", paste(countries, collapse = " ")))
...
# calculate distances ----
    ...
    if (nrow(current_points)==0) {
      ...
      # replace main_rivers_df if any DHS has empty set with current river
      # also replace this if one of the rivers has common points
      main_rivers_df[j,2] <- 1
      saveRDS(main_rivers_df, file = file.path(data_external_clean,"HydroSHEDS","main_rivers_checking_df.rds"))
      
      # iterate up because it's likely that no DHS year is going to have observations that work
      if (year == years[1]){
        i <- i+1 # iterate up the first time through if there aren't any points that intersect but not the other times otherwise it over-iterates
      }
      ...
      saveRDS(object = dyad_upstream,
              file = file.path(out_path,paste0("DHS_",year,"_",paste(countries,collapse="_"),"_MAIN_RIV_",main_river,"_dyad_distances.rds")))
      
      saveRDS(object = dyad_distances,
              file = file.path(out_path,paste0("DHS_",year,"_",paste(countries,collapse="_"),"_MAIN_RIV_",main_river,"_distances_matrix.rds")))
      
      print(paste0("Saved distance matrices at ",out_path,"/",paste0("DHS_",year,"_",paste(countries,collapse="_"),"_MAIN_RIV_",main_river,"_dyad_distances.rds")))
...
# plot current river, and current river in the countries ----
      ...
      print("Finished plotting, doing garbage cleanup")
    ...
    toc() # this logs time for running through an entire loop of main_river 
...
# extra plots ----
  ...
  save_map(output_folder = out_path,
           plotname = map,
           filename = paste0(main_river,paste(countries,collapse = "_"),year,"_with_rivers.png"),
           width = 8,
           height = 10,
           dpi  = 300)
...
# plot of entire hydronetwork over multiple countries ----
  ...
  # just do the plot with the DHS the first time we go through the rivers
    ...
    map <- ggplot(data = units) +
      ...
      geom_sf(data = closest_points)+
      labs(title = paste0("Survey Clusters and Snapping to River Segments ",paste(countries,collapse = " "), ", DHS ",year),
           caption = c(paste0("Snapped points in red; original cluster in black; buffer of ",max_distance_to_snap/1000," km around DHS clusters.
                              Data from DHS (",year, "), GADM (2022), HydroRIVERS (2023).")),
           colour = "Elevation (m)")+
      xlab("") + ylab("")+
      theme_map(#legend_position = "none",
        axis_text_x = element_blank(),
