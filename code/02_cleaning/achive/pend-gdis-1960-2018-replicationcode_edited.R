...
## for versions of EM-DAT data that added the ISO3 country code to the iso_a3, use this code to remove the ISO3 code to enable merge with GDIS
## rename the variable "Dis No" from EMDAT and remove the three-letter ISO from the iso_a3 identifier
emdat <- disasterlist%>%
  mutate(iso_a3=substr(`Dis No`,1,nchar(`Dis No`)-4)) 

## otherwise rename the variable "Dis No" from EMDAT without removing any characters
emdat <- disasterlist%>%
  mutate(iso_a3=`Dis No`) 
...
gdis_geo$continent <- countrycode(sourcevar = gdis_geo[, "country"],
                            origin = "country.name",
...
# Add year from emdat via iso_a3-identifier
gdis_geo_yr <- left_join(gdis_geo,disasteryear, by="iso_a3")


##### MANUSCRIPT FIGURES #####
## ____________________________________________________________________________________ ##
## Figure 1. Disaster locations by disaster type, 1960-2018
fig1<-ggplot(data=gdis_geo, aes(x=disastertype)) +
  geom_bar(fill="#2D708EFF") +
  labs(x = "",
       y = "Number of locations",
       title = "") +
  scale_y_continuous(labels = function(gdis_geo) format(gdis_geo, big.mark = " ",scientific = FALSE)) +
...
fig3a<-ggplot(data=gdis_geo_yr, aes(x=year, fill=factor(disastertype))) +
  geom_bar() +
  labs(x = "",  
...
## ____________________________________________________________________________________ ##
## Figure 1. Disaster locations by disaster type, 1960-2018
fig1_c<-ggplot(data=gdis_geo, aes(x=gdis_geo$disastertype)) +
  geom_bar(fill="#2D708EFF") +
  labs(x = "",
       y = "Number",
       title = "") +
...
fig2_c<-ggplot(data=gdis_geo, aes(x=gdis_geo$level)) +
  geom_bar(fill="#2D708EFF") +
  labs(x = "",
       y = "Number",
       title = "") +
...
t2 <- gdis_geo %>%
  group_by(country) %>%
  summarize(unique_disasters=n_distinct(iso_a3))
