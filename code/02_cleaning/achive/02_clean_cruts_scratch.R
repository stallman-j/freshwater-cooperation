...
true_annuals <- annuals_sf 
true_temp_Sf <- temp_sf
...
for (year in years){
  ...
  #           st_drop_geometry()  %>% as.numeric()
  # 
  # for just those columns in our year of interest, generate a rowmean
  my_cols <- temp_sf[,relevant_cols] %>%
              st_drop_geometry() %>%
  ...
  annuals_sf <- cbind(current_mean = my_cols$mean,
                      current_sum  = my_cols$sum,
                           annuals_sf)
  
  # assign the name to what was just "current_mean"
  name_mean <- paste0(year,"_mean")
  name_sum  <- paste0(year,"_sum")
  
  names(annuals_sf)[1] <- my_name
  
}

# add back in countrynames

# this is for droughts data
  # covariates <- sp_shapefile %>%
  #                    st_as_sf() %>%
  #                    st_drop_geometry()
