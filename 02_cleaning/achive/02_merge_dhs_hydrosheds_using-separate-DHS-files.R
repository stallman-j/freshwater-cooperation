...
# bring in hydrorivers ----
  ...
  gps_datasets     <- dhs_datasets() %>%
                      ...
                      filter(FileType == "Geographic Data") # this is the basic data
  ...
  # 10027014 is number 697
  
  long_rivers <- integer()   
  
  long_rivers <- c(long_rivers,10428478)
  # some Senegal rivers and their segment counts
  # 10428478 has 20441 segments: it's the river Senegal
  # 10489092 has 1448 segments: the river Gambia?
  # 10461041 has 21 segments
  # 10475466 has 21 
  # 10478774 has 27
  # 10489691 has 23
  # 10508130 has 72
  ...
  #main_river <- main_rivers_all[1]
  

  # started 2023-12-06 at around 16:20pm
  
# let's parallelize making the river network
...
# parallelize getting the river networks ----
...
# get the DHS locations placed on river networks ----
  ...
  for (main_river in main_rivers_all){
    ...
    if (!file.exists(file = file.path(data_external_clean,"HydroSHEDS","river_networks",paste0("MAIN_RIV_",main_river,"_cleaned_river_network.rds")))){
      ...
      current_river_network <- readRDS(file = file.path(out_path,paste0("MAIN_RIV_",main_river,"_cleaned_river_network.rds")))
      
    # get the external boundary of the river 
      # TO DO: MAKE SURE THIS WORKS
    polygon_boundary <- current_river_network %>% st_buffer(20000) %>% st_convex_hull() %>% st_union() 
    ...
    intersecting_indices <- lengths(st_intersects(gadm_data, single_river))>0
    ...
    desired_datasets   <- dhs_datasets() %>%
      filter(DatasetType == "GPS Datasets") %>% 
      filter(FileType == "Geographic Data") %>% 
      filter(DHS_CountryCode %in% countries_DHS)%>% 
      filter(SurveyYear >= surveyyear_start & SurveyYear <= surveyyear_end) %>%
      filter(SurveyType == "DHS")
    
    if (nrow(desired_datasets)==0){
      print(paste0("No DHS datasets within the desired survey years for ",paste(countries,collapse = " ")))
      i <- i+1
    } else{ 
    ...
    #source_segment <- single_river %>% filter(NEXT_DOWN ==0)

    dhs_gps_filenames  <- stringr::str_extract(desired_datasets$FileName,"[^.]+")
    #dhs_gps_filename_rds <- paste0(dhs_gps_filename,".rds")
    
    dhs_gps_in         <- file.path(data_external_temp,"DHS","GPS",
                                    paste0(dhs_gps_filenames,"_GADM_ADM_",level,".rds"))
  
# bring in DHS datasets ----
    
    dhs_data <- readRDS(dhs_gps_in[1]) %>%
      filter(!is.na(GID_0)) %>% # take out the ones that couldn't merge onto the country level
      st_transform(crs = equal_area_crs) %>%
      select(DHSID,DHSCC,DHSYEAR,DHSCLUST,CCFIPS,ADM1DHS,ADM1NAME,DHSREGCO,DHSREGNA,URBAN_RURA,LATNUM,LONGNUM,ALT_GPS,ALT_DEM,
             DATUM,geometry,GID_0,COUNTRY,NAME_1,TYPE_1,vector_sf_id,vector_cast_id,elevation,elev_units)
    
    # merge on all the relevant datasets
    for (i in 2:length(dhs_gps_in)){
      
      # bring in dataset and merge it if it exists
      
      if (!file.exists(dhs_gps_in[i])){print(paste0("File ",dhs_gps_in[i]," not found. Check your stuff."))} # if dhs_gps_filename doesn't exist
        
        else {
        
        dhs_data_tmp <- readRDS(dhs_gps_in[i]) %>%
          filter(!is.na(GID_0)) %>% # take out the ones that couldn't merge onto the country level
        st_transform(crs = equal_area_crs) %>%
          select(DHSID,DHSCC,DHSYEAR,DHSCLUST,CCFIPS,ADM1DHS,ADM1NAME,DHSREGCO,DHSREGNA,URBAN_RURA,LATNUM,LONGNUM,ALT_GPS,ALT_DEM,
                 DATUM,geometry,GID_0,COUNTRY,NAME_1,TYPE_1,vector_sf_id,vector_cast_id,elevation,elev_units)
        
        dhs_data <- rbind(dhs_data,dhs_data_tmp)
      } 
    }
...
# calculate distances ----
    ...
    if (nrow(current_points)==0) {
      print(paste0("No points in current DHS ",dhs_gps_filenames[i], " match with the current choice of river ID #,",main_river, ". Keep looping."))
      } else{
...
# plot current river, and current river in the countries ----
      ...
      map <- ggplot() +
        ...
        geom_sf(data = source_stretch,
                color = 'green',
                linewidth = 1.5)+
        labs(title = paste0("Example: River Distance Calculations,", paste(countries,collapse = " ")," ", surveyyear_start,"-",surveyyear_end," DHSs"),
             caption = c(paste0("Reported cluster in black; buffer of 5 km around cluster in dashed red; river source in green
                             ...
                             Data from DHS (",surveyyear_start,"-",surveyyear_end,")", "GADM (2022), AWS (2023), HydroRIVERS (2023).")),
             colour = "Elevation (m)")+
      ...
      save_map(output_folder = out_path,
               plotname = map,
               filename = paste0(paste(countries,collapse = "_"),"_MAIN_RIV_",main_river,"_example_river_distances.png"),
               width = 8,
               height = 7,
               dpi  = 300)
      ...
      save_map(output_folder = out_path,
               plotname = map,
               filename = paste0(paste(countries,collapse = "_"),"_MAIN_RIV_full_",main_river,"_example_river_distances.png"),
               width = 8,
               height = 7,
               dpi  = 300)
