...
# choose countries or rivers to include ----
  ...
  for (filename in distances_filenames){
    ...
    if (filename == distances_filenames[1]){
      ...
      if (number_of_towns>3) { # ignoring big rivers
  ...
  saveRDS(dyads, file = file.path(out_path,"dyads-towns.rds"))
  ...
  get_dhs_rivers_points <- function(main_river,
    ...
    tryCatch( {
      ...
      if (file.exists(file = file.path(checked_river_path,paste0("MAIN_RIV_",main_river,"_already_checked.rds")))){
        ...
        stop(paste0("DHS river points for main_river ",main_river," have already been calculated. No need to go through this again."))
...
# parallelize ----
  ...
  # 16 cores puts CPU close to 100% and memory at about 60 with singletons, probably not the best
  ...
  parLapply(cl, main_rivers_singletons, get_dhs_rivers_points)
  parLapply(cl, main_rivers_dyads, get_dhs_rivers_points)
  parLapply(cl, main_rivers_triads, get_dhs_rivers_points)
